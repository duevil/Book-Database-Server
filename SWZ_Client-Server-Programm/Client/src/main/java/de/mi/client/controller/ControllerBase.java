...
abstract class ControllerBase {
    ...
    protected ControllerBase() {
        LOGGER.debug("Creating new {}", getClass().getSimpleName());
        ...
        selectedBook.addListener((observable, oldValue, newValue) -> {
            LOGGER.debug("Selected book changed from {} to {}", oldValue, newValue);
    ...
    protected final void selectionAction(boolean isUpdate, boolean isDelete, boolean isCreate) {
        try {
            ...
            LOGGER.warn("Invalid book value(s) for action", e);
    ...
    protected final void loadBooks(boolean filter) {
        LOGGER.debug("Loading books with filter: {}", filter);
    ...
    private void updateBook() {
        switch (state.get()) {
            ...
            case UPDATING -> {
                LOGGER.debug("Updating book: {}", selectedBookProperties.get());
    ...
    private void createBook() {
        switch (state.get()) {
            ...
            case CREATING -> {
                LOGGER.debug("Creating book: {}", selectedBookProperties.get());
    ...
    private void deleteBook() {
        if (state.get() == State.NONE) {
            LOGGER.debug("Deleting book: {}", selectedBookProperties.get());
    ...
    private void alertCantPerformAction(String reason) {
        LOGGER.debug("Can't perform action: {}", reason);
